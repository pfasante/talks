...
\begin{frame}{Differential Cryptanalysis}{One round}
    ...
    \only<2->{\note{\begin{itemize}
        ...
        \item Remember that for decryption we need that $x$ and $x+k$ are mapped to the same value by $f_k$
...
\begin{frame}{Addressing Rationale 1}{The Key Schedule}
    ...
    \end{minipage}
    \only<1->{\note{\begin{itemize}
        \item Due to further analysis, we chose $3n$ rounds.
    ...
    \begin{minipage}{0.47\textwidth}
        ...
        \begin{block}{Construction}
            \begin{itemize}
                ...
                \item \only<1>{$\mathcal{O}(n)$ rounds?}\only<2->{$\mathcal{O}(n) = 3n$ rounds}
            \end{itemize}
        \end{block}
    \end{minipage}
    }
