...
\begin{frame}{Block Ciphers}
    ...
    \begin{scope}
        ...
        \node (m0) [above of=f0, node distance=2.25cm] {$p_0 \in \set{0,1}^n$};
        \node (k0) [left of=f0, node distance=2.75cm] {$k \in \set{0,1}^n$};
        \node (c0) [below of=f0, node distance=2.25cm] {$c_0 \in \set{0,1}^n$};
        \draw[-latex, thick] (m0) -- (f0);
        \draw[-latex, thick] (k0) -- (f0);
        \draw[-latex, thick] (f0) -- (c0);

        \node (title) [above of=m0, xshift=0pt, yshift=0pt, align=center] {Encrypt plaintext in blocks $p_i$ of $n$ bits,\\under a key of $n$ bits:};
    \end{scope}
    \visible<2->{%
    \begin{scope}
