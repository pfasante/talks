...
\begin{frame}{Differential Cryptanalysis}{More rounds}
    ...
    \only<1->{\note{\begin{itemize}
        ...
        \item Starting with the input difference $\alpha$, choosing an input value $x$ determines the path we take through the tree.
        \item Now assuming that $\alpha = k_1 + k_2$ it can not happen that the differential characteristic takes twice the right branch.
        \item This would result in a zero difference, which is not possible for permutations as long as the input difference is non-zero.
    \end{itemize}}}
    \only<2->{\note{\begin{itemize}
