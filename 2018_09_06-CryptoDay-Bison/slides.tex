...
\begin{frame}{Generic Analysis}{On the Boolean functions $f$}
    ...
    \begin{exampleblock}{Rationale 2}
        For any instance, $f$ has to depend on all bits, and for any $\delta \in \F_2^n:\ \Pr\bracket{f_k(x) = f_k(x + \delta)} \approx \frac{1}{2}$.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Addressing Rationale 2}{The Round Function}
    ...
    \begin{minipage}{0.985\textwidth}
    \begin{exampleblock}{Rationale 2}
        For any instance, the $f$ should depend on all bits, and for any $\delta \in \F_2^n:\ \Pr\bracket{f_k(x) = f_k(x + \delta)} \approx \frac{1}{2}$.
    \end{exampleblock}
    \end{minipage}
