...
\begin{frame}{Generic Analysis}{On the number of rounds}
    ...
    \begin{forest}
        ...
        \draw[decorate,decoration={brace,amplitude=3pt}]
            (right node.south east) -- (left node.south west) node[midway, below=4pt] {$\makebox[6pt][c]{$c$} \in \Span\set{k_1, k_2, k_3} + \makebox[6pt][c]{$p$}$};
        }
    \end{forest}
    }\only<2>{%
    \begin{forest}
    ...
    \begin{forest}
        %for tree={grow=east},
        [{\color{alertred}$\makebox[6pt][c]{$\alpha$}$\vphantom{p}}, name=root
            [{$\makebox[6pt][c]{$\alpha$}$\vphantom{p}}
                [{$\makebox[6pt][c]{$\alpha$}$\vphantom{p}}
                    [{$\makebox[6pt][c]{$\alpha$}$\vphantom{p}}, name=left node]
                    [{$\makebox[6pt][c]{$\alpha$} + k_3$\vphantom{p}}]
                ]
                [{$\makebox[6pt][c]{$\alpha$} + k_2$\vphantom{p}}
                    [{$\makebox[6pt][c]{$\alpha$} + k_2$\vphantom{p}}]
                    [{$\makebox[6pt][c]{$\alpha$} + k_2 + k_3$\vphantom{p}}]
                ]
            ]
            [{\color{alertred}$\makebox[6pt][c]{$\alpha$} + k_1$\vphantom{p}}, edge={thick, alertred}
                [{$\makebox[6pt][c]{$\alpha$} + k_1$\vphantom{p}}
                    [{$\makebox[6pt][c]{$\alpha$} + k_1$\vphantom{p}}]
                    [{$\makebox[6pt][c]{$\alpha$} + k_1 + k_3$\vphantom{p}}]
                ]
                [{\color{alertred}$\makebox[6pt][c]{$\alpha$} + k_1 + k_2$\vphantom{p}}, edge={thick, alertred}
                    [{\color{alertred}$\makebox[6pt][c]{$\alpha$} + k_1 + k_2$\vphantom{p}}, edge={thick, alertred}]
                    [{$\makebox[6pt][c]{$\alpha$} + k_1 + k_2 + k_3$\vphantom{p}}, name=right node]
                ]
            ]
        ]
        \node[above of=root, yshift=-15pt] {Input Differences};
        \visible<2->{%
        \draw[decorate,decoration={brace,amplitude=3pt}]
            (right node.south east) -- (left node.south west) node[midway, below=4pt] {$\makebox[6pt][c]{$\beta$} \in \Span\set{k_1, k_2, k_3} + \makebox[6pt][c]{$\alpha$}$};
        }
    \end{forest}
    }
    \endgroup
    ...
    \vspace{-5pt}
    \begin{equation*}
        E_{k,w}(p) \coloneqq c = p + \sum_{i=1}^{r} \lambda_i k_i
    \end{equation*}
    \end{block}
    \end{minipage}
    }
    \visible<4->{%
    \begin{minipage}[t][55pt][t]{0.47\textwidth}
    \begin{block}{Input/Output Differences}
        \vspace{5pt}
        For pairs $p_i, c_i$: $\Span \set{p_i + c_i} \subseteq \Span \set{k_j}$.
        \vspace{5pt}
    \end{block}
    \end{minipage}
    }
...
\begin{frame}{A genus of the WSN construction: BISON}
    \begin{block}{Generic properties of {\textbf{B}ent wh\textbf{I}tened \textbf{S}wap \textbf{O}r \textbf{N}ot}}
        ...
        \begin{itemize}
            ...
            \item All derivatives are balanced (bent)
        \end{itemize}
        \end{minipage}
        \vspace{5pt}
    \end{block}
...
\begin{frame}{Differential Cryptanalysis}{Primer}
    \begin{minipage}{0.47\textwidth}
        ...
        \begin{equation*}
            \Prob\bracket{E_k(x) + E_k(x + \alpha) = \beta} = p_{E_k}(\alpha, \beta) =\ \text{?}
        \end{equation*}
        Often key-alternating cipher:
        \begin{equation*}
            E_k(x) = (+_{k_n} \circ R \circ \cdots \circ +_{k_2} \circ R \circ +_{k_1})(x)
        \end{equation*}
